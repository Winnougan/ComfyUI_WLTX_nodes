...
Clones Kijai's LTX2SamplingPreviewOverride node (OuterSampleCallbackWrapper)
with fire skin on the frontend. All preview logic is straight from KJNodes.
"""
...
NODE_NAME = "Winnougan LTX Latent Preview"

serv = server.PromptServer.instance

# ── Latent → RGB projection matrices (straight from KJNodes) ──────────────────

LATENT_RGB_FACTORS_20 = [
...
class WrappedPreviewer:
    ...
    def decode_latent_to_preview_image(self, preview_format, x0):
        ...
        # Use .start() not .run() — must be non-blocking
        Thread(
    ...
    def process_previews(self, image_tensor, ind, leng):
        ...
        if image_tensor.size(1) > max_size or image_tensor.size(2) > max_size:
            ...
            if image_tensor.size(2) < image_tensor.size(3):
                ...
                image_tensor = F.interpolate(
                    image_tensor, (height, max_size), mode="nearest"
                )
            else:
                width = (max_size * image_tensor.size(3)) // image_tensor.size(2)
                image_tensor = F.interpolate(
                    image_tensor, (max_size, width), mode="nearest"
                )
            image_tensor = image_tensor.movedim(0, -1)

        previews_ubyte = (image_tensor.clamp(0, 1).mul(0xFF)).to(
    ...
    def decode_latent_to_preview(self, x0):
        if self.taeltx is not None:
            ...
            x0 = x0.unsqueeze(0).to(dtype=dtype, device=device).clone()
            with torch.no_grad():
            ...
            # Match VHS: linear projection output lives in roughly -1..1,
            # so shift-and-scale rather than sigmoid for correct colours.
            latent_image = (latent_image + 1.0) / 2.0
            return latent_image


# ── OUTER_SAMPLE wrapper — uses ComfyUI native previewer, same as Kijai ───────
...
class OuterSampleCallbackWrapper:
    ...
    def __call__(self, executor, noise, latent_image, sampler, sigmas,
        ...
        # Previewer is built lazily on the first callback, at which point
        # serv.last_node_id is guaranteed to point to our node.
        previewer_holder = [None]

        pbar  = comfy.utils.ProgressBar(total_steps)
        start = time.time()

        def new_callback(step, x0, x, total_steps_cb):
            # ── Lazy previewer init ──────────────────────────────────────────
            if previewer_holder[0] is None:
                latent_fmt   = guider.model_patcher.model.latent_format
                num_channels = getattr(latent_fmt, "latent_channels", None)
                if self.taeltx and self.vae is not None:
                    previewer_holder[0] = WrappedPreviewer(
                    ...
                    # 128-channel latent → LTXV 0.9.7 matrix, else 0.9.5
                    if num_channels == 128:
                        factors, bias = LATENT_RGB_FACTORS_23, LATENT_RGB_BIAS_23
                    else:
                        factors, bias = LATENT_RGB_FACTORS_20, LATENT_RGB_BIAS_20
                    previewer_holder[0] = WrappedPreviewer(
                ...
                log.info(
                    f"[{NODE_NAME}] previewer initialised: "
                    f"taeltx={self.taeltx}, channels={num_channels}, "
                    f"node_id={serv.last_node_id}"
                )

            previewer = previewer_holder[0]

            x0_work = x0
            if x0_work is not None and shape is not None:
                cut     = math.prod(shape[1:])
                x0_work = x0_work[:, :, :cut].reshape([x0_work.shape[0]] + list(shape)[1:])
            if num_keyframes > 0 and x0_work is not None:
                x0_work = x0_work[:, :, :-num_keyframes]

            preview_bytes = None
            if previewer and x0_work is not None:
                preview_bytes = previewer.decode_latent_to_preview_image("JPEG", x0_work)

            pbar.update_absolute(step + 1, total_steps_cb, preview_bytes)

            elapsed = time.time() - start
            eta = (elapsed / (step + 1)) * (total_steps_cb - step - 1) if step >= 0 else 0
            serv.send_sync("winnougan_ltx_preview", {
                "step":    step + 1,
                "total":   total_steps_cb,
                "elapsed": round(elapsed, 1),
                "eta":     round(eta, 1),
            })
            if callback is not None:
                callback(step, x0, x, total_steps_cb)

        out = executor(noise, latent_image, sampler, sigmas, denoise_mask,
                       new_callback, disable_pbar, seed, latent_shapes=latent_shapes)
...
class WinnouganLTXLatentPreview:
    """
    Clones Kijai's LTX2SamplingPreviewOverride exactly:
      - MODEL in  →  MODEL out
      - optional VAE input  (pass TAEHV for fast decoded previews)
      - optional LatentUpscaleModel input
      - preview_rate int
    """
        ...
        return {
            "required": {
                ...
                "preview_rate": ("INT", {"default": 8, "min": 1, "max": 60, "step": 1,
                                         "tooltip": "Preview frames per second target."}),
            },
            "optional": {
                "vae":                  ("VAE",),
    ...
    def patch(self, model, preview_rate, vae=None, latent_upscale_model=None):
        taeltx = False
        if vae is not None:
            vae_class = vae.first_stage_model.__class__.__name__
            if vae_class in ("TAEHV", "TAESD", "TAESDDecoder", "TAESDXL"):
                taeltx = True
                latent_upscale_model = None   # TAEHV handles its own decode
            log.info(f"[{NODE_NAME}] VAE class: {vae_class}, taeltx={taeltx}")

        model_clone = model.clone()
        wrapper = OuterSampleCallbackWrapper(
        ...
        model_clone.add_wrapper_with_key(
            ...
            "winnougan_ltx_preview",
            wrapper,
        )
        log.info(f"[{NODE_NAME}] OUTER_SAMPLE patched, rate={preview_rate}")
        return (model_clone,)


NODE_CLASS_MAPPINGS = {
    "WinnouganLTXLatentPreview": WinnouganLTXLatentPreview,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "WinnouganLTXLatentPreview": "🔥 Winnougan LTX Latent Preview",
}
